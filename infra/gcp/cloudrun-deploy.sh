...
SERVICE_NAME="${CLOUDRUN_SERVICE_NAME:-sooda-date-backend}"
SA_NAME="${CLOUDRUN_SA_NAME:-sooda-date-backend-sa}"
SA_EMAIL="$SA_NAME@$PROJECT_ID.iam.gserviceaccount.com"
CLOUDSQL_INSTANCE_NAME="${CLOUDSQL_INSTANCE_NAME:-sooda-date-db}"
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --set-secrets="DATABASE_URL=sooda-date-database-url:latest,SECRET_KEY=sooda-date-secret-key:latest,GOOGLE_OAUTH_CLIENT_ID=sooda-date-google-oauth-client-id:latest,KAKAO_REST_API_KEY=sooda-date-kakao-rest-api-key:latest" \
  --set-secrets="/secrets/firebase-service-account.json=sooda-date-firebase-credentials:latest" \
