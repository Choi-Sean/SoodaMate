...
const translations = {
  ...
  "terms.a10.body": {
    ko: "앱 내 결제(인앱결제)를 통한 구매는 App Store 또는 Google Play의 환불 정책 및 절차에 따릅니다. 웹사이트를 통해 직접 결제한 경우, 결제일로부터 7일 이내이며 서비스가 실제로 사용되지 않은 건에 한해 제11조 문의처로 요청 시 환불이 가능하며, 이미 소비된 크레딧이나 이용 기간이 경과한 구독은 환불되지 않을 수 있습니다.",
    en: "Purchases made via in-app purchase follow the refund policies and procedures of the App Store or Google Play. For purchases made directly through the website, a refund may be requested via the contact information in Article 11 within 7 days of payment, provided the service has not actually been used; already-consumed credits or subscription periods that have already elapsed may not be eligible for a refund.",
    es: "Las compras realizadas mediante compra dentro de la app siguen las políticas y procedimientos de reembolso de la App Store o Google Play. Para las compras realizadas directamente a través del sitio web, se puede solicitar un reembolso mediante la información de contacto del Artículo 11 dentro de los 7 días posteriores al pago, siempre que el servicio no se haya utilizado realmente; los créditos ya consumidos o los períodos de suscripción ya transcurridos pueden no ser reembolsables.",
    zh: "通过应用内购买的商品，遵循App Store或Google Play各自的退款政策与流程。通过网站直接购买的，如服务实际尚未使用，可在付款后7天内通过第11条联系方式申请退款；已消耗的积分或已过期的订阅期间可能无法退款。",
    ja: "アプリ内課金（IAP）による購入は、App StoreまたはGoogle Playの返金ポリシーおよび手続きに従います。ウェブサイトを通じて直接決済した場合、サービスが実際に利用されていない場合に限り、決済日から7日以内に第11条の連絡先を通じて返金を請求できます。既に消費されたクレジットや期間が経過したサブスクリプションは返金の対象外となる場合があります。",
  },
