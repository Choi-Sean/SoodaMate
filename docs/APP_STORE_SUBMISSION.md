...
- Bundle ID: `com.sudalist.sudadate` (must match `mobile/app.config.js`'s `ios.bundleIdentifier`)
- Name: 수다 데이트 (or the localized name you want on the store)
...
- Package name: `com.sudalist.sudadate` (must match `mobile/app.config.js`'s `android.package`)
- App category: Dating
