...
Both (plus email) return the same `TokenResponse` shape. If the provider's email matches an existing account, the new provider is linked to that account instead of creating a duplicate user.
