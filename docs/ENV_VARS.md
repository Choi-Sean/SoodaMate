...
| `EXPO_PUBLIC_ADMOB_ANDROID_NATIVE_UNIT_ID` / `EXPO_PUBLIC_ADMOB_IOS_NATIVE_UNIT_ID` | AdMob console → Ad units, after creating a **Native Advanced** unit per platform (not Banner/Interstitial/Rewarded/App Open — `src/components/AdCard.native.tsx` only implements the native in-swipe-deck sponsored card) | **iOS set** — **Android still on Google's test unit ID** |
