...
async def presign_face_photo(
    ...
    object_path = storage_service.build_face_verification_object_path(user.id, body.content_type, body.kind)
    return {
...
async def submit_face_verification(
    ...
    if existing is not None:
        existing.gcs_object_path = body.selfie_object_path
        ...
        row = FaceVerification(
            user_id=user.id,
            gcs_object_path=body.selfie_object_path,
            id_photo_object_path=body.id_photo_object_path,
            status="pending",
        )
        db.add(row)
    await db.commit()
